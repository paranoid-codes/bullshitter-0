...
export default function UploadDropzone({ onFilesSelected }: UploadDropzoneProps) {
  ...
  const filterFiles = (files: File[]) => {
    const valid = files.filter((f) => f.name.endsWith(".filter"))
    const invalid = files.length - valid.length
    if (invalid > 0) {
      showError(`Only .filter files are allowed. Ignored ${invalid} file${invalid > 1 ? "s" : ""}.`)
    }
    return valid
  }
  ...
  return (
    <div>
      ...
      >
        ...
        <h3 className="text-lg font-medium mb-2">Drop .filter files here to upload</h3>
        ...
        <p className="text-xs text-muted-foreground">Only .filter files are supported</p>
        <input
          ref={fileInputRef}
          type="file"
          multiple
